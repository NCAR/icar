...
! 		define the dimensions
		call check( nf90_def_dim(ncid, "x", nx, x_id) )
		dimids(1)=x_id
		call check( nf90_def_dim(ncid, "z", nz, temp_id) )
		dimids(2)=temp_id
		call check( nf90_def_dim(ncid, "y", ny, y_id) )
		dimids(3)=y_id
		call check( nf90_def_dim(ncid, "xu", nx+1, xu_id) )
		call check( nf90_def_dim(ncid, "yv", ny+1, yv_id) )
