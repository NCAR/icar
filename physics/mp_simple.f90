...
!                   endif
            ...
            if (qv<qvsat) then
                if (qr>SMALL_VALUE) then
                    ! evaporate rain
                    call phase_change(pressure,temperature,qr,qvsat,qv,L_evap,cloud2rain)
                endif
                if (qs>SMALL_VALUE) then
                    ! sublimate snow
                    call phase_change(pressure,temperature,qs,qvsat,qv,L_subl,cloud2snow)
                endif
            endif
        endif
    end subroutine
...
!       remove that from the bottom model layer.
    ...
    !!  @param nz         = number of vertical grid cells - 0D - input  - n      - scalar
                ...
                do i = 1,nz
                    ...
                    if (qv(i)<qvsat) then
                        if (qr(i)>SMALL_VALUE) then
                            ! evaporate rain
                            call phase_change(pressure(i),temperature(i),qr(i),qvsat,qv(i),L_evap,cloud2rain)
                        endif
                    endif
                enddo
            enddo
                ...
                do i = 1,nz
                    ...
                    if (qv(i)<qvsat) then
                        if (qs(i)>SMALL_VALUE) then
                            ! sublimate snow
                            call phase_change(pressure(i),temperature(i),qs(i),qvsat,qv(i),L_subl,cloud2snow)
                        endif
                    endif
                enddo
            enddo
...
!       enddo
    ...
    !!  @param dt         = time step                     - 0D - input  - sec.   - scalar
    !!  @param nx         = number of ew grid cells       - 0D - input  - n      - scalar
    !!  @param ny         = number of ns grid cells       - 0D - input  - n      - scalar
    !!  @param nz         = number of vertical grid cells - 0D - input  - n      - scalar
    !!
    !!----------------------------------------------------------
...
!   real,dimension(5) ::pressure,temperature,qv,qc,qr,qs,dz
...
!       write(*,*) temperature(2),qv(2),qc(2),qr(2),qs(2),rain,snow
