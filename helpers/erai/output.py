...
def write_file(date,info,erai):
    ...
    extra_vars.append(Bunch(name="level",data=erai["z"],dims=dims,dtype="f",attributes=atts))
    ...
    extra_vars.append(Bunch(name="lat",data=info.lat_data,dims=dims[2:],dtype="f",attributes=atts))
    
    atts=Bunch(long_name="longitude",units="degrees")
    extra_vars.append(Bunch(name="lon",data=info.lon_data,dims=dims[2:],dtype="f",attributes=atts))
    ...
    atts=Bunch(long_name="time",units="days since 1900-01-01")
