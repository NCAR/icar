...
submodule(boundary_interface) boundary_implementation
    ...
    use icar_constants
    use data_structures,        only : geo_look_up_table, vert_look_up_table
...
contains
    ...
    module subroutine init(this, options)
      ...
      call bc_init(this%current_domain, this, options)
    ...
    module subroutine update_forcing(this, options)
        ...
        call bc_update(this%current_domain, this, options)
    ...
    module subroutine distribute_update(this)
      ...
      call distribute_domain(this%next_domain)
    ...
    module subroutine distribute_initial_conditions(this)
      ...
      call distribute_domain(this%current_domain)

    end subroutine

    subroutine distribute_domain(domain)
        implicit none
        type(domain_type), intent(inout) :: domain

    end subroutine distribute_domain
...
!       interpolate data onto the high resolution grid
    ...
    subroutine remove_linear_winds(domain,bc,options,filename,curstep)
        implicit none
        type(domain_type), intent(inout) :: domain
        class(boundary_t), intent(inout) :: bc
        type(options_t),intent(in) :: options
        ...
        ! load low-res U data
        call io_read(filename,options%parameters%uvar,extra_data,curstep)
        ...
        ! load low-res V data
        call io_read(filename,options%parameters%vvar,extra_data,curstep)
        nx=size(extra_data,1)
        ny=size(extra_data,2)
        nz=size(extra_data,3)
        call smooth_array(extra_data,1,2)
...
!       load low-res V data
    ...
    subroutine bc_init(domain,bc,options)
        ...
        nfiles = size(options%parameters%boundary_files)
        allocate(file_list(nfiles))
        file_list = options%parameters%boundary_files

        if (options%parameters%restart) then
            call set_curfile_curstep(options, options%parameters%restart_time)
        else
            call set_curfile_curstep(options, options%parameters%start_time)
        endif


        if (.not.options%parameters%ideal) then
            smoothing_window = min(max(int(options%parameters%smooth_wind_distance/domain%dx),1),size(domain%lat,1)/5)
            if (options%parameters%debug) write(*,*) "  Smoothing winds over ",trim(str(smoothing_window))," grid cells"
        endif
!       load the restart file
        if (options%parameters%restart) then
            call load_restart_file(domain,options%parameters%restart_file,options%parameters%restart_step_in_file)
            write(*,*) "Reading winds"
            write(*,*) "  timestep:",trim(str(curstep)),"  from file:",trim(file_list(curfile))
            ! if (options%parameters%external_winds) then
            ...
            ! elseif (options%parameters%mean_winds) then
            !     call mean_winds(domain,file_list(curfile),curstep,options)
            else
                call read_var(domain%u, file_list(curfile),options%parameters%uvar,  &
                                bc%u_geo%geolut,bc%u_geo%vert_lut,curstep,boundary_value, &
                                options)
                call read_var(domain%v, file_list(curfile),options%parameters%vvar,  &
            ...
            if (options%parameters%time_var=="") then
                domain%model_time = options%parameters%start_time
            else
                call read_bc_time(domain%model_time, file_list(curfile), options%parameters%time_var, curstep)
            endif
...
!           else load data from the first Boundary conditions file
            ...
            call read_var(domain%p,    file_list(curfile),   options%parameters%pvar,   &
                            bc%geolut, bc%vert_lut, curstep, boundary_value, &
                            options,   bc%lowres_z,domain%z, interp_vertical=.True.)

            call read_var(domain%th,   file_list(curfile),   options%parameters%tvar,   &
                            bc%geolut, bc%vert_lut, curstep, boundary_value, &
                            options)
            ...
            call read_var(domain%qv,   file_list(curfile),   options%parameters%qvvar,  &
                            bc%geolut, bc%vert_lut, curstep, boundary_value, &
                            options)
            ...
            if (trim(options%parameters%qcvar)/="") then
                call read_var(domain%cloud,file_list(curfile),   options%parameters%qcvar,  &
                                bc%geolut, bc%vert_lut, curstep, boundary_value, &
                                options)
            else
                if (options%parameters%debug) write(*,*) "No Cloud water content specified"
                domain%cloud=0
            endif
            if (trim(options%parameters%qivar)/="") then
                call read_var(domain%ice,  file_list(curfile),   options%parameters%qivar,  &
                                bc%geolut, bc%vert_lut, curstep, boundary_value, &
                                options)
            else
                if (options%parameters%debug) write(*,*) "No Cloud ice specified"
                domain%ice=0
            endif

            if (trim(options%parameters%shvar)/="") then
                call read_2dvar(domain%sensible_heat,file_list(curfile),options%parameters%shvar,  bc%geolut,curstep)
            endif
            if (trim(options%parameters%lhvar)/="") then
                call read_2dvar(domain%latent_heat,  file_list(curfile),options%parameters%lhvar,  bc%geolut,curstep)
            ...
            if (trim(options%parameters%pblhvar)/="") then
                call read_2dvar(domain%pbl_height,   file_list(curfile),options%parameters%pblhvar,bc%geolut,curstep)
            endif

            if (trim(options%parameters%swdown_var)/="") then
                call read_2dvar(domain%swdown,  file_list(curfile),options%parameters%swdown_var,  bc%geolut,curstep)
            endif
            if (trim(options%parameters%lwdown_var)/="") then
                call read_2dvar(domain%lwdown,  file_list(curfile),options%parameters%lwdown_var,  bc%geolut,curstep)
            endif

            if (trim(options%parameters%sst_var)/="") then
                call read_2dvar(domain%sst,  file_list(curfile), options%parameters%sst_var,  bc%geolut,curstep)
            endif
            if (trim(options%parameters%rain_var)/="") then
                call read_2dvar(bc%drain_dt, file_list(curfile), options%parameters%rain_var, bc%geolut,curstep)
            endif
...
!!------------------------------------------------------------
    ...
    subroutine bc_update(domain,bc,options)
        ...
        type(options_t),intent(in)::options
        integer,dimension(io_maxDims)::dims !note, io_maxDims is included from io_routines.
        type(boundary_t) :: newbc ! just used for updating z coordinate
        ...
        if (options%parameters%time_var=="") then
            bc%next_domain%model_time = bc%next_domain%model_time + options%parameters%input_dt
        else
            call read_bc_time(bc%next_domain%model_time, file_list(curfile), options%parameters%time_var, curstep)
        endif
        ...
        if (options%parameters%time_varying_z) then
            ...
            call io_read(file_list(curfile), options%parameters%zvar, newbc%z, curstep)
            nx=size(newbc%z,1)
            ny=size(newbc%z,2)
            nz=size(newbc%z,3)
            if (trim(options%parameters%zbvar)/="") then
                call io_read(file_list(curfile),options%parameters%zbvar, zbase, curstep)
        ...
        elseif (options%parameters%mean_winds) then
            ...
            call read_var(bc%next_domain%u,  file_list(curfile), options%parameters%uvar, &
                          bc%u_geo%geolut,   bc%u_geo%vert_lut,  curstep,      &
                          use_interior, options, time_varying_zlut=newbc%vert_lut)
            call read_var(bc%next_domain%v,  file_list(curfile), options%parameters%vvar, &
        ...
        call read_var(bc%next_domain%p,       file_list(curfile), options%parameters%pvar,   &
        ...
        call read_var(bc%next_domain%th,      file_list(curfile), options%parameters%tvar,   &
                      bc%geolut, bc%vert_lut, curstep, use_interior,              &
                      options, time_varying_zlut=newbc%vert_lut, interp_vertical=.True. )
        ...
        call read_var(bc%next_domain%qv,      file_list(curfile), options%parameters%qvvar,  &
                      bc%geolut, bc%vert_lut, curstep, use_boundary,              &
                      options, time_varying_zlut=newbc%vert_lut)
        ...
        if (trim(options%parameters%qcvar)/="") then
            call read_var(bc%next_domain%cloud,   file_list(curfile), options%parameters%qcvar,  &
                          bc%geolut, bc%vert_lut, curstep, use_boundary,              &
                          options, time_varying_zlut=newbc%vert_lut)
        endif
        if (trim(options%parameters%qivar)/="") then
            call read_var(bc%next_domain%ice,     file_list(curfile), options%parameters%qivar,  &
                          bc%geolut, bc%vert_lut, curstep, use_boundary,              &
                          options, time_varying_zlut=newbc%vert_lut)
        endif
        ...
        if (options%physics%landsurface==kLSM_BASIC) then
            call read_2dvar(bc%next_domain%sensible_heat,file_list(curfile),options%parameters%shvar,  bc%geolut,curstep)
            call read_2dvar(bc%next_domain%latent_heat,  file_list(curfile),options%parameters%lhvar,  bc%geolut,curstep)
            ...
            if (options%physics%boundarylayer==kPBL_BASIC) then
                if (trim(options%parameters%pblhvar)/="") then
                    call read_2dvar(bc%next_domain%pbl_height,   file_list(curfile),options%parameters%pblhvar,bc%geolut,curstep)
                endif
            endif
            ! NOTE, this is a kludge to prevent the model from sucking more moisture out of the lower model layer than exists
            where(domain%latent_heat<0) domain%latent_heat=0
        ...
        if (options%physics%radiation==kRA_BASIC) then
            if (trim(options%parameters%swdown_var)/="") then
                call read_2dvar(bc%next_domain%swdown,  file_list(curfile),options%parameters%swdown_var,  bc%geolut,curstep)
            endif
            if (trim(options%parameters%lwdown_var)/="") then
                call read_2dvar(bc%next_domain%lwdown,  file_list(curfile),options%parameters%lwdown_var,  bc%geolut,curstep)
            endif
        endif

        if (trim(options%parameters%sst_var)/="") then
            call read_2dvar(bc%next_domain%sst,  file_list(curfile),options%parameters%sst_var,  bc%geolut,curstep)
        endif
        if (trim(options%parameters%rain_var)/="") then
            call read_2dvar(bc%drain_dt,  file_list(curfile),options%parameters%rain_var,  bc%geolut,curstep)
        endif
        ...
        if (options%parameters%mean_fields) then
